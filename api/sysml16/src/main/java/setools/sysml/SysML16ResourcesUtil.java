 ...
 *
 */
public class SysML16ResourcesUtil extends UMLUtil {
	...
	*/
	public static Map<URI, URI> initURIConverterURIMap(Map<URI, URI> uriMap) {
		...
		mapURIs(uriMap, "platform:/plugin/org.eclipse.papyrus.sysml16/resources/", //$NON-NLS-1$
			umlURI.appendSegment("profile"), "org.eclipse.papyrus.sysml16"); //$NON-NLS-1$ //$NON-NLS-2$
		
		URI umlResourcesURI = getBaseSysMLResourcesURI();
		mapURIs(uriMap, SysMLResource.PROFILES_PATHMAP,
			umlResourcesURI.appendSegment("profile"), //$NON-NLS-1$
			ResourcesPlugin.PLUGIN_ID);
		mapURIs(uriMap, SysMLResource.LIBRARIES_PATHMAP,
	...
	*/
	public static Map<String, URI> initEPackageNsURIToProfileLocationMap(
		...
		ePackageNsURIToProfileLocationMap.put(SysMLPackage.eNS_URI,
			URI.createURI("pathmap://SysML16_PROFILES/SysML.profile.uml#SysML")); //$NON-NLS-1$
	...
	private static URI getBaseSysMLURI() {
		URL resultURL = SysMLResource.class.getClassLoader()
			.getResource(String.format("resources/profile/%s", "sysml.ecore")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (resultURL != null) {
			...
			resultURL = SysMLResource.class.getResource("SysMLResource.class"); //$NON-NLS-1$
	...
	private static URI getBaseSysMLResourcesURI() {
		...
		URL resultURL = SysMLResource.class.getClassLoader().getResource(

 ...
 *
 */
public class MBPEResourcesUtil extends UMLUtil {
	...
	*/
	public static EPackage.Registry initPackageRegistry(
		...
		packageRegistry.put(MbpePackage.eNS_URI, MbpePackage.eINSTANCE);
	...
	*/
	public static ContentHandler.Registry initContentHandlerRegistry(
		...
		//TODO: do we need to register any content handlers?
	...
	*/
	public static Map<URI, URI> initURIConverterURIMap(Map<URI, URI> uriMap) {
		URI umlURI = getBaseMbpeURI();
		mapURIs(uriMap, "platform:/plugin/setools/resources/", //$NON-NLS-1$
			umlURI.appendSegment("profile"), "setools.mbpe"); //$NON-NLS-1$ //$NON-NLS-2$
		
		URI umlResourcesURI = getBaseMbpeResourcesURI();
		mapURIs(uriMap, MbpeResource.PROFILES_PATHMAP,
			umlResourcesURI.appendSegment("profile"), //$NON-NLS-1$
			ResourcesPlugin.PLUGIN_ID);
		mapURIs(uriMap, MbpeResource.LIBRARIES_PATHMAP,
			umlResourcesURI.appendSegment("library"), //$NON-NLS-1$
			ResourcesPlugin.PLUGIN_ID);
	...
	*/
	public static Map<String, URI> initEPackageNsURIToProfileLocationMap(
		...
		ePackageNsURIToProfileLocationMap.put(MbpePackage.eNS_URI,
			URI.createURI(MbpeResource.PROFILE_PATH)); //$NON-NLS-1$
		
		ePackageNsURIToProfileLocationMap.put(MbpeResource.PROFILE_URI,
				URI.createURI("pathmap://SysML14_PROFILES/SysML.profile.uml#SysML")); //$NON-NLS-1$
		return ePackageNsURIToProfileLocationMap;
	}

	private static URI getBaseMbpeURI() {
		URL resultURL = MbpeResource.class.getClassLoader()
			.getResource(String.format("resources/profile/%s", "mbpe.ecore")); //$NON-NLS-1$ //$NON-NLS-2$
		
		URI result;
		
		if (resultURL != null) {
			// remove the /profile/sysml.ecore segments of the resource
			// we found
			...
			resultURL = MbpeResource.class.getResource("MbpeResource.class"); //$NON-NLS-1$
	...
	private static URI getBaseMbpeResourcesURI() {
		URI umlMetamodel = URI.createURI(MbpeResource.PROFILE_PATH);
		URL resultURL = MbpeResource.class.getClassLoader().getResource(
			String.format("resources/profile/%s", umlMetamodel.lastSegment())); //$NON-NLS-1$
		
		URI result;
		
		if (resultURL != null) {
			// remove the /profile/SysML.profile.uml segments of the resource
			// we found
			result = URI.createURI(resultURL.toExternalForm(), true)
				.trimSegments(2);
		} else {
			// probably, we're not running with JARs, so assume the source
			// project folder layout
			resultURL = MbpeResource.class
				.getResource("MbpeResource.class"); //$NON-NLS-1$
